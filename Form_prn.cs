                                    ...
                                    + "("
                                        + "("
                                            + "(t_seikyu INNER JOIN t_syubetsu"
                                                ...
                                                + " AND t_seikyu.o_id::Integer = t_syubetsu.o_id"
                                            + ")"
                ...
                case 4: // 納入案内
                    {
                        ...
                        + " LEFT JOIN (SELECT req_id, title1, title2, title3, title4, name1, name2, data7, data8, title4_kana, name2_kana FROM t_req) f ON a.req_id::Integer = f.req_id)"
                        + " LEFT JOIN (SELECT o_id, o_name, o_manager, o_stuff, o_phone_number FROM t_office) f1 ON a.o_id = f1.o_id)"
                        ...
                        + ") "
                        + " LEFT JOIN (SELECT stf_id, stuff, start_date, end_date, o_id FROM t_stuff) j ON j.o_id::Text= '" + Form_Seikyu_TextBoxO_id + "'"
                        + " AND j.start_date <= (SELECT to_date((to_number(c4_y,'999')+diff || '/' || to_number(c4_m,'99')+1 || '/' || to_number('01','99')),'yyyy/mm/dd')-1)"
                        + " AND (j.end_date Is Null Or j.end_date >= (SELECT to_date((to_number(c4_y,'999')+diff || '/' || to_number(c4_m,'99')+1 || '/' || to_number('01','99')),'yyyy/mm/dd')-1) "
                ...
                case '0': // 
                    {
                        ...
                        + "WITH RECURSIVE r as ("
                            ...
                            + " ON h.c9 = b.b_code, t_req s, t_office o, t_syubetsu x"
                            + " WHERE x.o_id = h.o_id"
