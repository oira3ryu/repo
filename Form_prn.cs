                ...
                case 1: // 納額通知書
                    {
                       ...
                       + "with recursive r as ("
                        ...
                        + ", i.end_date"
                        + ", i.diff"
                        + " from h inner join t_gengou i on substr(h.c4_y,1,1) = i.g_name;"
                        , m_conn
                        );
                ...
                case 3: // 収入内訳
                    {
                        ...
                        (
                            ...
                            + ", to_number(r.c4_m,'99')::text as tsuki"
                            + " from r"
                            + " group by r.c4, r.s_id, r.o_id, r.c4_y, r.c4_m, r.req_id, r.syubetsu, r.tsuki, r.time_stamp"
                ...
                case 4: // 納入案内
                    {
                        ...
                        + ", case when e.sy IS NULL or e.sm IS NULL then" /* 引落年月日自動入力の場合*/
                            + " case when e.b_id = '0' then null"
                                + " when e.b_id = '1' then" /* 稚内信金の場合の処理*/
                                    ...
                                    + " else" /* 翌月末引落以外の場合の処理 */
                                            + " target_date(to_date((to_number(c4_y,'999')-12+2000 || '/' || to_number(c4_m,'99')+1 || '/' || to_number(e.sd,'99')),'yyyy/mm/dd'))"
                                    + " end"
                        ...
                        + " else" /* 引落年月日手入力の場合*/
                            + " case when e.b_id = '0' then null"
                                + " when e.b_id = '1' then" /* 稚内信金の場合の処理*/
                                    + " case when e.sd = '99' then" /* 翌月末引落の場合の処理 */
                                        + " target_date(to_date((to_number(e.sy,'999')-12+2000 || '/' || to_number(e.sm,'99')+2 || '/' || to_number('01','99')),'yyyy/mm/dd')-1)"
                                    + " else" /* 翌月末引落以外の場合の処理 */
                                        + " target_date(to_date((to_number(e.sy,'999')-12+2000 || '/' || to_number(e.sm,'99') || '/' || to_number(e.sd,'99')),'yyyy/mm/dd'))"
                                    + " end"
                                + " when e.b_id = '2' then" /* ゆうちょ銀行の場合の処理*/
                                    + " case when e.sd = '99' then" /* 翌月末引落の場合の処理 */
                                        + " target_date(to_date((to_number(e.sy,'999')-12+2000 || '/' || to_number(e.sm,'99')+2 || '/' || to_number('01','99')),'yyyy/mm/dd')-1)"
                        ...
                        + " where time_stamp = (select max(time_stamp) from t_shiharai_houhou where"
                            ...
                            + " and s_id::Integer = " + Cmb_s_id_int
                            + " and o_id::Integer = " + Cmb_o_id_int
                        + ")) b on a.c1 = b.c5 and rtrim(replace(a.c3,substr(a.c3,strpos(a.c3, '('), strpos(a.c3, ')')),'')) = rtrim(replace(b.c6,substr(b.c6,strpos(b.c6, '('), strpos(b.c6, ')')),'')) and a.s_id = b.s_id and a.o_id = b.o_id and b.s_id::Integer = " + Cmb_s_id_int + ")"
                        ...
                        + " and i.o_id::Integer = " + Cmb_o_id_int
                        + " order by a.id"
                        + ")"
                        + " select"
                        + " 利用者番号"
                        ...
                        + ", j.end_date"
                        + ", j.diff"
                        + " from r inner join t_gengou j on substr(r.c4_y,1,1) = j.g_name;"
                      , m_conn
                        );
                ...
                case 5: // 引落請求書
                    {
                        ...
                        + ", i.end_date"
                        + ", i.diff"
                        + " from r inner join t_gengou i on substr(r.c4_y,1,1) = i.g_name;"
                                , m_conn
                            );
